...
def TifCrop(TifPath, SavePath, CropSize, RepetitionRate): # tif文件裁剪函数，输入tif文件路径，保存路径，裁剪尺寸和重复率
    ...
    # 获取数据 Readasarray支持按块读取栅格，可以针对dataset也可以针对波段band,xoff = 0，yoff= 0xoff与yoff指定想要读取的部分原点位置在整张图像中距离全图的原点位置,
    # 这样就是读取全部数组信息了,GDAL读取的原点为左上角位置
...
TifCrop(r"D:\JavaConsist\MapData\out1part2.tif", r"D:\JavaConsist\MapData\out", 2560, 0)